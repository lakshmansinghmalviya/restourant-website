  ...
  return (
    <div className="bg-gradient-to-br from-red-800 via-red-700 to-red-900 py-12 sm:py-16 lg:py-20 px-4 sm:px-6 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-10 left-10 w-24 sm:w-32 h-24 sm:h-32 bg-red-600/20 rounded-full blur-3xl animate-pulse"></div>
        <div className="absolute bottom-20 right-20 w-32 sm:w-40 h-32 sm:h-40 bg-orange-500/15 rounded-full blur-3xl animate-pulse delay-1000"></div>
        <div className="absolute top-1/2 left-1/4 w-20 sm:w-24 h-20 sm:h-24 bg-yellow-500/10 rounded-full blur-2xl animate-pulse delay-500"></div>
      </div>
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="text-center mb-12 sm:mb-16">
          <div className="inline-flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
            <div className="p-2.5 sm:p-3 bg-gradient-to-r from-orange-500 to-red-600 rounded-full shadow-lg">
              <Users className="w-6 sm:w-8 h-6 sm:h-8 text-white" />
            </div>
            <div className="h-px w-12 sm:w-16 bg-gradient-to-r from-orange-400 to-transparent"></div>
            <Flame className="w-5 sm:w-6 h-5 sm:h-6 text-orange-400" />
            <div className="h-px w-12 sm:w-16 bg-gradient-to-l from-orange-400 to-transparent"></div>
          </div>
          ...
          <div className="mt-6 sm:mt-8 w-24 sm:w-32 h-1 bg-gradient-to-r from-orange-400 to-red-500 mx-auto rounded-full"></div>
        </div>
        ...
        <div className="relative">
          ...
          <button
            onClick={prevSlide}
            className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-20 w-12 lg:w-14 h-12 lg:h-14 bg-white/20 backdrop-blur-sm rounded-full items-center justify-center text-white hover:bg-white/30 transition-all duration-300 shadow-lg hover:scale-110"
          >
          ...
          <button
            onClick={nextSlide}
            className="hidden md:flex absolute right-0 top-1/2 -translate-y-1/2 z-20 w-12 lg:w-14 h-12 lg:h-14 bg-white/20 backdrop-blur-sm rounded-full items-center justify-center text-white hover:bg-white/30 transition-all duration-300 shadow-lg hover:scale-110"
          >
            <ChevronRight className="w-5 lg:w-6 h-5 lg:h-6" />
          </button>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8 px-0 md:px-12 lg:px-16">
            {teamMembers.map((member, index) => (
              <div
                key={member.id}
                className={`group relative transition-all duration-700 ease-out ${
                  index === currentSlide ? 'md:scale-105 z-10' : 'md:scale-95 md:opacity-75'
                }`}
                onMouseEnter={() => setHoveredCard(member.id)}
                onMouseLeave={() => setHoveredCard(null)}
              >
                <div className="relative bg-white rounded-xl sm:rounded-2xl overflow-hidden shadow-2xl transition-all duration-500 hover:shadow-3xl">
                  <div className="relative h-80 sm:h-96 overflow-hidden">
                    ...
                    <div
                      className={`absolute top-3 sm:top-4 right-3 sm:right-4 z-20 transition-all duration-500 ${
                        hoveredCard === member.id
                          ? 'opacity-100 translate-y-0'
                          : 'opacity-0 translate-y-2'
                      }`}
                    >
                      <div className="bg-gradient-to-r from-orange-500 to-red-600 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 flex items-center gap-2 shadow-lg">
                        <Star className="w-3 sm:w-4 h-3 sm:h-4 text-white fill-current" />
                        <span className="text-xs sm:text-sm font-bold text-white">{member.experience}</span>
                      </div>
                    </div>

                    <div
                      className={`absolute top-3 sm:top-4 left-3 sm:left-4 z-20 transition-all duration-500 delay-100 ${
                        hoveredCard === member.id
                          ? 'opacity-100 translate-y-0'
                          : 'opacity-0 translate-y-2'
                      }`}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 z-20 p-4 sm:p-6">
                      ...
                      <p className="text-red-200 font-medium text-base sm:text-lg">{member.title}</p>
                    </div>
                  </div>

                  {/* Hover content */}
                  <div
                    className={`absolute inset-0 bg-gradient-to-t from-red-900 via-red-800 to-red-700 flex flex-col justify-center items-center text-center p-6 sm:p-8 transition-all duration-500 ease-out ${
                  ...
                  >
                    <div className="transform transition-all duration-500 delay-200">
                      <h3 className="text-2xl sm:text-3xl font-bold text-white mb-2 sm:mb-3">{member.name}</h3>
                      <p className="text-orange-300 font-semibold text-base sm:text-lg mb-4 sm:mb-6">{member.title}</p>

                      <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                        <div className="flex items-center justify-center gap-3">
                          <Flame className="w-4 sm:w-5 h-4 sm:h-5 text-orange-400" />
                          <span className="text-white font-medium text-sm sm:text-base">{member.specialty}</span>
                        </div>
                        <div className="flex items-center justify-center gap-3">
                          <Award className="w-4 sm:w-5 h-4 sm:h-5 text-yellow-400" />
                          <span className="text-white font-medium text-sm sm:text-base">{member.achievement}</span>
                        </div>
                      </div>
                ...
                <div
                  className={`absolute inset-0 rounded-xl sm:rounded-2xl bg-gradient-to-r from-orange-500/20 to-red-600/20 transition-opacity duration-500 pointer-events-none blur-xl -z-10 ${
